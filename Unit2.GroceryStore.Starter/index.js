...
const capitalNameArray = items.map((eachItem)=> eachItem.name.toUpperCase())
...
const idName = items.find((x)=> x.id=== id)
return idName

  // TODO: use `find`
}

// const findOurItem = (items, id) => {
  // const foundItem = items.find((val)=>val.c === 3)
// }
...
function getItemPriceByName(items, name) {
  const priceName = items.find((x)=> x.name ===name)
...
function getItemsByCategory(items, category) {
  const finditems = items.filter((x)=> x.category ===category)
  return finditems
  // TODO: use `filter`
}
...
const numberofItems = items.reduce((x,y)=> x +=y.quantity,0)
...
const priceTotal = items.reduce ((x,y)=> x += (y.price * y.quantity),0)
return priceTotal
